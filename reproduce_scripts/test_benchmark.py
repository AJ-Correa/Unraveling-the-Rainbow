...
from utils.my_utils import benchmark_pomo_starting_nodes
    ...
    if not test_paras["sample"]:
        for instance in test_files:
            if test_paras["pomo_starting_nodes"]:
                initial_actions, env = benchmark_pomo_starting_nodes(instance, copy.deepcopy(env_test_paras), is_public_jssp)
                env.reset()
                first_timestep = True
            else:
