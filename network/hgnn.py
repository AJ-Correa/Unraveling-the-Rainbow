...
class GATedge(nn.Module):
    ...
    def __init__(self,
        ...
        self.is_tuple = isinstance(in_feats, tuple)
        self.residual = residual

        if self.is_tuple:
            if noisy:
                self.fc_src = NoisyLinear(
                    self._in_src_feats, out_feats * num_head)
                self.fc_dst = NoisyLinear(
                    self._in_dst_feats, out_feats * num_head)
                self.fc_edge = NoisyLinear(
                    1, out_feats * num_head)
            else:
        ...
        if self.residual:
            if self._in_dst_feats != out_feats:
                if noisy:
                    self.res_fc = NoisyLinear(
                        self._in_dst_feats, num_head * out_feats)
                else:
    ...
    def reset_noise(self):
        if self.is_tuple:
            self.fc_src.reset_noise()
            self.fc_dst.reset_noise()
            self.fc_edge.reset_noise()
        else:
            self.fc.reset_noise()

        if self.residual:
            if self._in_dst_feats != self._out_feats:
                self.res_fc.reset_noise()
    ...
    def __init__(self,
        ...
        if noisy:
            self.project = nn.Sequential(
                NoisyLinear(self._in_feats, hidden_dim),
                nn.ELU(),
                NoisyLinear(hidden_dim, hidden_dim),
                nn.ELU(),
                NoisyLinear(hidden_dim, self._out_feats),
            )
        else:
            self.project = nn.Sequential(
        ...
        if self.residual:
            if self._in_dst_feats != out_feats:
                if noisy:
                    self.res_fc = NoisyLinear(
                        self._in_dst_feats, self._num_heads * out_feats)
                else:
                    self.res_fc = nn.Linear(
