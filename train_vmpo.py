...
def main(env_paras, model_paras, train_paras):
    ...
    memories = VMPO_model.Memory()
    model = VMPO_model.VMPO(model_paras, train_paras, num_envs=train_paras["parallel_iter"])
    start_epoch = 1
    best_model = deque()
    current_model = deque()
    ...
    if train_paras["save_output"]:
        ...
        if model_paras["load_state"]:
            ...
            best_vmpo_model = VMPO_model.VMPO(model_paras, train_paras, num_envs=train_paras["parallel_iter"])
            best_vmpo_model.policy_old.load_state_dict(checkpoint_best_model)
            makespan_best, _ = validate(env_valid_paras, env_valid, best_vmpo_model.policy_old)
            del best_vmpo_model
