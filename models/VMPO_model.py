...
from network.mlp import MLPCritic, MLPActor


def get_KL(old_dist_prob, dist_prob):
    eps = 1e-10
    old_dist_prob = old_dist_prob + eps
    dist_prob = dist_prob + eps
    kl = old_dist_prob * (torch.log(old_dist_prob) - torch.log(dist_prob))
...
class HGNNScheduler(nn.Module):
    ...
    def act(self, state, memories, dones, epsilon=None, flag_sample=True, flag_train=True):
    ...
    def evaluate(self, ope_ma_adj, ope_pre_adj, ope_sub_adj, raw_opes, raw_mas, proc_time,
        ...
        return action_logprobs, state_values.squeeze().double(), dist_probs
...
class VMPO:
    def __init__(self, model_paras, train_paras, num_envs=None):
        ...
        self.eta = torch.tensor([train_paras["init_eta"]], requires_grad=True).to(self.device)
        self.alpha = torch.tensor([train_paras["init_alpha"]], requires_grad=True).to(self.device)
        self.eps_eta = torch.FloatTensor([train_paras["eps_eta"]]).to(self.device)
        self.eps_alpha = torch.FloatTensor(train_paras["eps_alpha"]).log().to(self.device)
    ...
    def update(self, memory, env_paras, train_paras):
        ...
        with torch.no_grad():
            _, old_state_values, old_dist_probs = \
                self.policy_old.evaluate(old_ope_ma_adj,
                                         old_ope_pre_adj,
                                         old_ope_sub_adj,
                                         old_raw_opes,
                                         old_raw_mas,
                                         ...
                                         old_eligible,
                                         old_action_envs)
            advantages = rewards_envs - old_state_values.detach()
        ...
        for _ in range(self.K_epochs):
            for i in range(num_complete_minibatches + 1):
                ...
                logprobs, state_values, dist_probs = \
                ...
                advprobs = torch.stack((batch_advantages, logprobs))
                advprobs = advprobs[:, torch.sort(advprobs[0], descending=True).indices]
                good_advantages = advprobs[0, :old_ope_ma_adj[start_idx: end_idx, :, :].size(0) // 2]
                good_logprobs = advprobs[1, :old_ope_ma_adj[start_idx: end_idx, :, :].size(0) // 2]

                # Get losses
                phis = torch.exp(good_advantages / self.eta.detach()) / torch.sum(
                    torch.exp(good_advantages / self.eta.detach()))
                L_pi = (-phis * good_logprobs).mean()
                L_eta = self.eta * self.eps_eta + self.eta * (good_advantages / self.eta).exp().mean().log()

                KL = get_KL(old_dist_probs[start_idx:end_idx, :].detach(), dist_probs)

                coef_alpha = torch.distributions.Uniform(self.eps_alpha[0], self.eps_alpha[1]).sample().exp()
                L_alpha = torch.mean(self.alpha * (coef_alpha - KL.detach()) + self.alpha.detach() * KL)

                loss = (L_pi + L_eta + L_alpha + 0.5 * self.MseLoss(state_values,
                                                                    rewards_envs[start_idx:end_idx])).mean()
