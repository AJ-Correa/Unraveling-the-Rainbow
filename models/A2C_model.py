...
class HGNNScheduler(nn.Module):
    ...
    def evaluate(self, ope_ma_adj, ope_pre_adj, ope_sub_adj, raw_opes, raw_mas, proc_time,
        ...
        return action_logprobs, state_values.squeeze().float(), dist_entropys
...
class A2C:
    def __init__(self, model_paras, train_paras, num_envs=None):
        ...
        self.A_coef = train_paras["A_coef"]  # coefficient for policy loss
        self.entropy_coef = train_paras["entropy_coef"]  # coefficient for entropy term
        self.vf_coef = train_paras["vf_coef"]  # value loss coefficient
    ...
    def update(self, memory, env_paras, train_paras):
        ...
        ope_ma_adj = torch.stack(memory.ope_ma_adj, dim=0).transpose(0, 1).flatten(0, 1)
        ope_pre_adj = torch.stack(memory.ope_pre_adj, dim=0).transpose(0, 1).flatten(0, 1)
        ope_sub_adj = torch.stack(memory.ope_sub_adj, dim=0).transpose(0, 1).flatten(0, 1)
        raw_opes = torch.stack(memory.raw_opes, dim=0).transpose(0, 1).flatten(0, 1)
        raw_mas = torch.stack(memory.raw_mas, dim=0).transpose(0, 1).flatten(0, 1)
        proc_time = torch.stack(memory.proc_time, dim=0).transpose(0, 1).flatten(0, 1)
        jobs_gather = torch.stack(memory.jobs_gather, dim=0).transpose(0, 1).flatten(0, 1)
        eligible = torch.stack(memory.eligible, dim=0).transpose(0, 1).flatten(0, 1)
        ...
        action_envs = torch.stack(memory.action_indexes, dim=0).transpose(0, 1).flatten(0, 1)
        ...
        logprobs, state_values, dist_entropy = \
            self.policy.evaluate(ope_ma_adj,
                                 ope_pre_adj,
                                 ope_sub_adj,
                                 raw_opes,
                                 raw_mas,
        ...
        loss = self.A_coef * -(logprobs * advantages) + self.vf_coef * self.MseLoss(state_values, returns) - self.entropy_coef * dist_entropy
        loss = loss.mean()
        ...
        avg_reward = discounted_total.item() / (self.num_envs * train_paras["update_timestep"])
