...
def main(env_paras, model_paras, train_paras, extension_paras):
    ...
    if not extension_paras["use_n_step"]:
        extension_paras["n_step_horizon"] = 1
    else:
        memories_n = ReplayBuffer(max_size=train_paras["buffer_size"], batch_size=train_paras["minibatch_size"],
                                  gamma=train_paras["gamma"], n_step_horizon=extension_paras["n_step_horizon"], use_n_step=True, to_cpu=model_paras["replay_to_cpu"])
    ...
    if extension_paras["use_per"]:
        ...
        memories = ReplayBuffer(max_size=train_paras["buffer_size"], batch_size=train_paras["minibatch_size"],
                                gamma=train_paras["gamma"], n_step_horizon=1, use_n_step=extension_paras["use_n_step"], to_cpu=model_paras["replay_to_cpu"])
    ...
    for i in range(start_epoch, train_paras["epochs"] + 1):
        ...
        while ~done:
            ...
            if extension_paras["use_n_step"]:
                one_step_transition = memories_n.store(model.online_network.transition)
            else:
            ...
            if len(memories.ope_ma_adj) >= train_paras["minibatch_size"]:
                if extension_paras["use_n_step"]:
                    model.update_model(memories, memories_n)
                else:
