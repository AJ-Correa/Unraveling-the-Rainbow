...
def main(env_paras, model_paras, train_paras):
    ...
    memories = A2C_model.Memory()
    model = A2C_model.A2C(model_paras, train_paras, num_envs=train_paras["parallel_iter"])
    start_epoch = 1
    best_model = deque()
    current_model = deque()
    ...
    if train_paras["save_output"]:
        ...
        if model_paras["load_state"]:
            ...
            best_a2c_model = A2C_model.A2C(model_paras, train_paras, num_envs=train_paras["parallel_iter"])
            best_a2c_model.policy.load_state_dict(checkpoint_best_model)
            makespan_best, _ = validate(env_valid_paras, env_valid, best_a2c_model.policy)
            del best_a2c_model
