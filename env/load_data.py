...
def load_fjs(lines, num_mas, num_opes, is_public_jssp=False):
    '''
    ...
    if is_public_jssp:
        ...
        for line in lines[:1 + int(num_opes / num_mas)]:
            # first line
            if flag == 0:
                flag += 1
            # last line
            ...
            # other
                ...
                num_ope = edge_detec_jssp(line, num_ope_bias, matrix_proc_time, matrix_pre_proc, matrix_cal_cumul, num_mas, current_job)
                current_job += 1
...
def nums_detec_jssp(lines):

    num_jobs, num_mas = map(int, lines[0].strip().split())
    num_opes = int(num_jobs * num_mas)
...
def edge_detec_jssp(line, num_ope_bias, matrix_proc_time, matrix_pre_proc, matrix_cal_cumul, num_mas, current_job):
    '''
    Detect information of a job
    '''
    line_split = line.split()
    ...
    for i in range(num_mas):
        matrix_proc_time[idx_ope + int(current_job * num_mas)][int(line_split[i * 2])] = int(line_split[i * 2 + 1])
        if idx_ope != num_mas - 1:
            matrix_pre_proc[idx_ope + num_ope_bias][idx_ope + num_ope_bias + 1] = True
        if idx_ope != 0:
            vector = torch.zeros(matrix_cal_cumul.size(0))
            vector[idx_ope + num_ope_bias - 1] = 1
