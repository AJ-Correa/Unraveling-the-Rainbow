...
from models import REINFORCE_model
...
def main(env_paras, model_paras, train_paras):
    ...
    memories = REINFORCE_model.Memory()
    model = REINFORCE_model.REINFORCE(model_paras, train_paras, num_envs=train_paras["parallel_iter"])
    start_epoch = 1
    best_model = deque()
    current_model = deque()
    ...
    if train_paras["save_output"]:
        ...
        if model_paras["load_state"]:
            ...
            best_reinforce_model = REINFORCE_model.REINFORCE(model_paras, train_paras, num_envs=train_paras["parallel_iter"])
            best_reinforce_model.policy.load_state_dict(checkpoint_best_model)
            makespan_best, _ = validate(env_valid_paras, env_valid, best_reinforce_model.policy)
            del best_reinforce_model
    ...
    for i in range(start_epoch, train_paras["max_iterations"] + 1):
        ...
        while ~done:
            with torch.no_grad():
                actions = model.policy.act(state, memories, dones)
        ...
        if i % train_paras["save_timestep"] == 0:
            ...
            vali_result, vali_result_100 = validate(env_valid_paras, env_valid, model.policy)
